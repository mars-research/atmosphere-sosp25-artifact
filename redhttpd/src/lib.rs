...
fn read_file(url: &[u8]) -> Option<&'static [u8]> {
    match url {
        ...
        b"/empty.bin" => Some(include_bytes!("htdocs/empty.bin")),
        _ => None,
    }
}
...
impl HttpSession {
    pub fn new<'a>(sockets: &mut SocketSet<'a>) -> Self {
        ...
        //let socket: tcp::Socket<'b, 'c> = socket.into();
    ...
    pub fn handle(&mut self, sockets: &mut SocketSet) {
        let mut socket = sockets.get_mut::<tcp::Socket>(self.handle);
    ...
    fn send_response(&mut self, socket: &mut tcp::Socket) {
        if !socket.may_send() {
            return;
        }
