        ...
        requires 
            ...
            forall|va:VAddr,pa:PAddr| #![auto] dom0_pagetable@@.value.get_Some_0().0.tmp_get_mem_mappings().contains_pair(va,pa) ==>
                (
                    va != 0
                    &&
                    page_ptr_valid(pa as usize)
                    &&
                    boot_page_ptrs@[page_ptr2page_index(pa as usize) as int].is_Some() && boot_page_ptrs@[page_ptr2page_index(pa as usize) as int].unwrap() == va
                ),
        ensures
            //self.kernel_wf(),
        {
