...
impl Kernel{
            ...
            old(self).mmu_man.get_pagetable_by_pcid(pcid).is_va_entry_exist(va)
    {
        ...
        // assert(self.page_alloc.get_available_pages() =~= old(self).page_alloc.get_available_pages());
        ...
        //     (
        //         old(self).page_alloc.get_available_pages().contains(old(self).mmu_man.get_pagetable_mapping_by_pcid(_pcid)[_va])
        //     )
        // ));
        ...
        //     (
        //         self.page_alloc.get_available_pages().contains(self.mmu_man.get_pagetable_mapping_by_pcid(_pcid)[_va])
        //     )
        // ));
        // assert(
        //     forall|_pcid:Pcid, _va:usize| #![auto] 0<=_pcid<PCID_MAX && spec_va_valid(_va) && self.mmu_man.get_pagetable_mapping_by_pcid(_pcid)[_va] != 0 && _pcid != pcid ==>
