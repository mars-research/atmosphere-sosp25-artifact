...
impl Kernel {
    pub fn syscall_send_endpoint_wait(&mut self, cpu_id:CPUID, pt_regs: Registers, endpoint_index: EndpointIdx, share_endpoint_index: EndpointIdx) -> (ret: SyscallReturnStruct)
        requires
            old(self).wf(),
        ensures
            self.wf(),
