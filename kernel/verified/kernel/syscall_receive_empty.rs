...
/// 
pub fn syscall_receive_empty_no_block(&mut self, receiver_thread_ptr: ThreadPtr, blocking_endpoint_index:EndpointIdx) ->  (ret: SyscallReturnStruct)
    requires
        old(self).wf(),
...
pub fn syscall_receive_empty_block(&mut self, receiver_thread_ptr: ThreadPtr, blocking_endpoint_index:EndpointIdx, pt_regs:&Registers) ->  (ret: SyscallReturnStruct)
    requires
        old(self).wf(),
        old(self).thread_dom().contains(receiver_thread_ptr),
        0 <= blocking_endpoint_index < MAX_NUM_ENDPOINT_DESCRIPTORS,
