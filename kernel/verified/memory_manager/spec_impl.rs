...
impl MemoryManager {
    ...
    pub open spec fn iommutables_wf(&self) -> bool {
        ...
        &&& forall|ioid: IOid|
            ...
            0 <= ioid < IOID_MAX ==> self.iommu_tables[ioid as int].is_None()
        ...
        &&& forall|ioid: IOid|
            ...
            0 <= ioid < IOID_MAX && self.iommu_tables[ioid as int].is_Some()
                ...
                && self.iommu_tables[ioid as int].unwrap().mapping_2m().dom() == Set::<
                VAddr,
            >::empty() && self.iommu_tables[ioid as int].unwrap().mapping_1g().dom() == Set::<
                VAddr,
            >::empty()
        &&& forall|ioid_i: IOid, ioid_j: IOid|
            ...
            0 <= ioid_i < IOID_MAX && self.iommu_tables[ioid_i as int].is_Some() && 0 <= ioid_j
                < IOID_MAX && self.iommu_tables[ioid_j as int].is_Some() && ioid_i != ioid_j
    ...
    pub open spec fn get_iomemory_managertable_page_closure_by_ioid(&self, ioid: IOid) -> Set<
        PagePtr,
    >
        recommends
            0 <= ioid < IOID_MAX,
    ...
    pub fn init(
        ...
        self.page_tables.set(
            0,
            Some(
                PageTable::new(0, self.kernel_entries_ghost, dom0_page_map_ptr, dom0_page_map_perm),
            ),
        );
    }
}
