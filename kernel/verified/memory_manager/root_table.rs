...
impl RootTable {
    ...
    pub fn init(&mut self)
        ensures
            ...
            forall|bus: u8, dev: u8, fun: u8|
                #![auto]
                0 <= bus < 256 && 0 <= dev < 32 && 0 <= fun < 8 ==> self.resolve(
                    bus,
                    dev,
                    fun,
                ).is_None(),
    ...
    pub closed spec fn wf(&self) -> bool {
        self.seq_ar@.len() == 256 && forall|bus: u8|
            #![auto]
            0 <= bus < 256 ==> self.seq_ar@[bus as int].len() == 32 && forall|bus: u8, dev: u8|
                #![auto]
                0 <= bus < 256 && 0 <= dev < 32 ==> self.seq_ar@[bus as int][dev as int].len() == 8
    }
            ...
            ret =~= self.resolve(bus, dev, fun),
    {
        ...
        let cr3 = self.deviecs[bus as usize].ar[((dev * fun) * 2 + 1) as usize]
            & 0xFFFFFFFFFFFFF000 as usize;
    ...
    pub fn set(&mut self, bus: u8, dev: u8, fun: u8, target: Option<(IOid, usize)>)
        ...
        ensures
            ...
            forall|_bus: u8, _dev: u8, _fun: u8|
                #![auto]
                0 <= _bus < 256 && 0 <= _dev < 32 && 0 <= _fun < 8 && (_bus != bus || _dev != dev
                    || _fun != fun) ==> self.resolve(_bus, _dev, _fun) =~= old(self).resolve(
                    _bus,
                    _dev,
                    _fun,
                ),
    {
        if target.is_none() {
            ...
            self.deviecs[bus as usize].ar[((dev * fun) * 2) as usize] = ((target.unwrap().0 << 8)
                & 0x7FFF00) | (0x010b);
            self.deviecs[bus as usize].ar[((dev * fun) * 2 + 1) as usize] = (target.unwrap().1
                & 0xFFFFFFFFFFFFF000) | (0x1001b);
        }
    }
}
