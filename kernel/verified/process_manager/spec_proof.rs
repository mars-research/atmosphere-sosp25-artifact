...
impl ProcessManager{
    ...
    pub open spec fn spec_get_endpoint_by_endpoint_idx(&self, thread_ptr:ThreadPtr, endpoint_index:EndpointIdx) -> Option<&Endpoint>
        recommends
            self.wf(),
            self.thread_dom().contains(thread_ptr),
            0 <= endpoint_index < MAX_NUM_ENDPOINT_DESCRIPTORS,
...
impl ProcessManager{
            ...
            self.get_endpoint(old(self).get_thread(thread_ptr).endpoint_descriptors@[endpoint_index as int].unwrap()).queue@ == old(self).get_endpoint(old(self).get_thread(thread_ptr).endpoint_descriptors@[endpoint_index as int].unwrap()).queue@.push(thread_ptr),
    {
        ...
        thread_set_blocking_endpoint_endpoint_ref_scheduler_ref_state_and_ipc_payload(
            thread_ptr, &mut thread_perm, Some(endpoint_ptr), Some(sll), None, ThreadState::BLOCKED, ipc_payload,
            Some(endpoint_index));
        proof {
            self.thread_perms.borrow_mut().tracked_insert(thread_ptr, thread_perm.get());
        }
    ...
    pub fn pass_endpoint(&mut self, src_thread_ptr:ThreadPtr, src_endpoint_index:EndpointIdx, dst_thread_ptr:ThreadPtr, dst_endpoint_index:EndpointIdx)
        ...
        ensures
            ...
            self.get_endpoint(old(self).get_thread(src_thread_ptr).endpoint_descriptors@[src_endpoint_index as int].unwrap()).owning_threads@ == old(self).get_endpoint(old(self).get_thread(src_thread_ptr).endpoint_descriptors@[src_endpoint_index as int].unwrap()).owning_threads@.insert((dst_thread_ptr,dst_endpoint_index)),
            ...
            self.get_endpoint(self.get_thread(src_thread_ptr).endpoint_descriptors@[src_endpoint_index as int].unwrap()).queue_state == old(self).get_endpoint(old(self).get_thread(src_thread_ptr).endpoint_descriptors@[src_endpoint_index as int].unwrap()).queue_state,
    {
        ...
        endpoint_add_ref(src_endpoint_ptr,&mut src_endpoint_perm, dst_thread_ptr, dst_endpoint_index);
        proof {
    ...
    pub fn new_endpoint(&mut self, thread_ptr:ThreadPtr, endpoint_index:EndpointIdx, page_ptr_1: PagePtr, page_perm_1: Tracked<PagePerm4k>)
        ...
        ensures
            ...
            self.get_endpoint(page_ptr_1).owning_threads@ =~= Set::<(ThreadPtr,EndpointIdx)>::empty().insert((thread_ptr, endpoint_index)),
            self.get_endpoint(page_ptr_1).owning_container =~= old(self).get_thread(thread_ptr).owning_container,
    {
        ...
        let (endpoint_ptr, endpoint_perm) = page_to_endpoint_with_thread_and_container(container_ptr, thread_ptr, endpoint_index, sll_index, page_ptr_1, page_perm_1);
        proof {
            self.endpoint_perms.borrow_mut().tracked_insert(endpoint_ptr, endpoint_perm.get());
        }
