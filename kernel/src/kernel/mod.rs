...
pub fn kernel_init(boot_page_ptrs: &ArrayVec<(u64,PhysicalMemoryType),{4*1024*1024}>, dom0_pagetable_ptr: usize, kernel_pml4_entry_ptr: usize){
    ...
    let mut old_kernel_pml4:usize = 0;
    unsafe{
        old_kernel_pml4 = *(dom0_pagetable_ptr as * const usize);
    }
    log::info!("old_kernel_pml4 {:x?}",old_kernel_pml4);

    let mut new_kernel_pml4:usize = 0;
    unsafe{
        new_kernel_pml4 = *(new_cr3 as * const usize);
    }
    log::info!("new_kernel_pml4 {:x?}",new_kernel_pml4);
