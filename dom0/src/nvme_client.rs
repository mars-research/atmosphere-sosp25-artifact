...
pub fn test_nvme_with_ring_buffer(){
    unsafe {
        ...
        //         return;
        //     }
        let error_code = asys::sys_new_thread(0,test_nvme_with_ring_buffer_ap as *const () as usize, rsp);
            if error_code != 0 {
                log::info!("sys_new_proc_with_iommu_pass_mem failed {:?}", error_code);
                return;
            }
        ...
        let mut nvme_dev =
            NvmeDevice::new(PciBarAddr::new(USERSPACE_BASE + NVME_BAR_BASE, 0x4000));
...
pub fn test_nvme_with_ring_buffer_pingpong_ap()-> ! {
    log::info!("hello from test_nvme_with_ring_buffer_pingpong_ap");
    ...
    unsafe{
        ...
        loop{
            ...
            asys::sys_send_empty_try_schedule(0);
        }
        let seq_read_end = _rdtsc();
...
pub fn test_nvme_pingpong(){
    unsafe {
        ...
        log::info!("Running Nvme Read/write tests!");
        let error_code = asys::sys_new_thread(0,test_nvme_with_ring_buffer_pingpong_ap as *const () as usize, rsp);
            if error_code != 0 {
                log::info!("sys_new_proc_with_iommu_pass_mem failed {:?}", error_code);
                return;
            }
